...
Figure \ref{fig:wkld-network-traffic} and Figure \ref{fig:global-channel-stime} shows that random job placement with adaptive routing can distribute workload traffic and balance the load over the network. Contiguous job placement 
...
\begin{figure*}[t!]
    ...
    \begin{subfigure}[t]{0.32\textwidth}
        \centering
        \includegraphics[height=1.8 in]{amg/commtime}
    ...
    \begin{subfigure}[t]{0.32\textwidth}
        \centering
        \includegraphics[height=1.8 in]{mg/commtime}
        \caption{MG Communication Time}
        \label{fig:mg-commtime}
    \end{subfigure}%
    \begin{subfigure}[t]{0.32\textwidth}
        \centering
        \includegraphics[height=1.8 in]{cr/commtime}
...
We look into network level to identify the major culprit behind AMG's abnormal behavior with random placement and adaptive routing. By identifying the terminal nodes and routers that each application's traffic go through, we can see that the traffic amount go through the routers that belongs to AMG is skyrocketing when it running with random placement and adaptive routing, shown in Figure \ref{fig:amg-router-traffic}. 

\begin{figure*}[t!]
    \centering
    \begin{subfigure}[t]{0.32\textwidth}
        \centering
        \includegraphics[height=1.8 in]{amg/tl-traffic}
        \caption{Terminal Link Traffic}
        \label{fig:amg-tl-traffic}
    \end{subfigure}%
    \hspace{1em}%
    \begin{subfigure}[t]{0.32\textwidth}
        \centering
    ...
    \end{subfigure}%
   \caption{AMG router traffic.}
   \label{fig:amg-router-traffic}
\end{figure*}

Adaptive routing will redirect the traffic of other applications to go through the routers belongs to AMG, and increase the burden of those routers, thus slow down the communication of AMG. 
