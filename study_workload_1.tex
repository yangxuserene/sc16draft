...
\begin{figure*}[t!]
    ...
    \end{subfigure}%
    \caption{Aggregate traffic and saturation time for Workload~\Rmnum{1} under the configurations listed in Table~\ref{tab: placement routing configs}. ``adp'' and ``padp'' have equivalent behavior.}
...
\begin{figure*}[t]
   ...
   \begin{subfigure}[t]{0.32\textwidth}
    ...
    \end{subfigure}
   \caption{Aggregate workload traffic for routers serving specific applications. ``adp'' and ``padp'' have equivalent behavior. More routers are involved in serving each application when random placement is in use, compared to contiguous placement.}
